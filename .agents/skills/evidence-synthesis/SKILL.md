...
6. Analyze accuracy/quality, efficiency, scalability, robustness, generalization, interpretability, privacy/security, engineering complexity, and artifact availability only where relevant. For gap analysis, test candidates against alternate terminology, adjacent subfields, recent proceedings, and citation networks.
7. Audit every consequential sentence against the claim ledger: confirm the cited source supports the exact claim, the locator was checked, the relevant text was accessed, inference is labeled, the version is clear, and bibliographic metadata agrees with an authoritative record.
