...
import qhist_audit_e0_v14 as audit


class QHistV14AuditTests(unittest.TestCase):
    def test_json_domain_accepts_only_serialization_type_equivalence(self):
        in_memory = {
            "milestone_mapping": {0: (15, 0.0), 1: (31, 1.0)},
            "similarity": 0.5,
            "turn_count": 3,
        }
        persisted = {
        ...
        self.assertEqual(audit.json_domain(in_memory), audit.json_domain(persisted))
        changed = dict(persisted, similarity=0.5000001)
        self.assertNotEqual(audit.json_domain(in_memory), audit.json_domain(changed))
