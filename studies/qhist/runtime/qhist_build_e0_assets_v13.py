#!/usr/bin/env python3
"""构建 Q-HIST v13 E0 的冻结 A/R/E 资产，不调用目标模型。"""
...
class FrozenUUID4:
    ...
    def __call__(self) -> uuid.UUID:
        ...
        return uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"QHIST-v13|{self.episode_id}|predecessor|{self.counter}",
        )
...
def gap_blocks() -> list[list[dict[str, Any]]]:
    ...
    for index, (record, value) in enumerate(values, start=1):
        call_id = f"call_qh13_external_register_{index}"
        blocks.append(
            [
                {
                    "role": "assistant",
...
def weekday_meta(tools: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, (tool, arguments, truth, false) in enumerate(
        ...
        truth_messages.extend(openai_call_pair("QH13", index, tool, arguments, truth))
        false_messages.extend(openai_call_pair("QH13", index, tool, arguments, false))
...
def contact_meta(tools: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    for index, (tool, arguments, true_result, false_result) in enumerate(calls, start=1):
        messages_true.extend(openai_call_pair("QH13", index, tool, arguments, true_result))
        messages_false.extend(openai_call_pair("QH13", index, tool, arguments, false_result))
...
def wifi_meta(tools: dict[str, Any]) -> dict[str, Any]:
    ...
    true_messages = openai_call_pair("QH13", 1, "get_low_battery_mode_status", {}, truth)
    false_messages = openai_call_pair("QH13", 1, "get_low_battery_mode_status", {}, False)
...
def build_episode(
    ...
    if kind == "weekday-target-timestamp-shift":
        ...
        raise RuntimeError(f"未知 v13 error_kind：{kind}")
    ...
    if len(messages) != 2 or [row.get("role") for row in messages] != ["system", "user"]:
        raise RuntimeError(f"v13 development 只接受信息完整单轮任务：{frozen['scenario']}")
...
def main() -> int:
    ...
    manifest = {
        ...
        "v13_spec_sha256": sha256_file(script_dir / "qhist_v13_spec.py"),
