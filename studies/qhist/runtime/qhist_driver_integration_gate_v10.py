#!/usr/bin/env python3
"""用固定动作代理验证 Q-HIST v10 driver、承诺状态与 reply-level fail-closed。"""
...
def main() -> int:
    ...
    predicates = {
        row["episode_id"]: row for row in read_jsonl(args.assets / "predicates.jsonl")
    }
    patches = {
        row["episode_id"]: row
    ...
    episode = episodes["QH10-E0-01"]
    predicate = predicates["QH10-E0-01"]
    ...
    def fixed_generate(endpoint, request_id, expected_precision, messages, tools, max_new_tokens):
        candidate = actions[cursor["value"]]
        cursor["value"] += 1
        raw = "<tool_call>\n" + json.dumps(candidate) + "\n</tool_call><|im_end|>"
        return generated_result(request_id, expected_precision, tools, raw, [candidate])

    runner.base.generate_decision = fixed_generate
    row = {
        "unit_id": "ENGINEERING-QH10-E0-01-P00-C_align-d3",
    ...
    def invalid_generate(endpoint, request_id, expected_precision, messages, tools, max_new_tokens):
        ...
        if index == 0:
            ...
            invalid = {"name": "qhist_nonexistent_tool", "arguments": {}}
            raw = "".join(
                "<tool_call>" + json.dumps(candidate) + "</tool_call>"
    ...
    invalid_row = dict(
        row,
        unit_id="ENGINEERING-QH10-E0-01-P00-C_text-d3-invalid",
        history="C_text",
    )
    ...
    checks = {
        ...
        "commitment_active_after_false_action": valid_result["decisions"][1][
            "commitment_debt"
        ],
        "pre_patch_environment_debt_detected": valid_result["decisions"][1][
            "environment_debt"
        ],
        "c_align_patch_changed_state": valid_result["correction"]["patch"]["changed"],
        "post_correction_verification_clears_commitment": not valid_result["decisions"][3][
            "commitment_debt"
        ],
        "final_commitment_residual_zero": valid_result["metrics"][
            "commitment_residual"
        ]
        == 0.0,
        "final_environment_residual_zero": valid_result["metrics"][
            "environment_residual"
        ]
        == 0.0,
        "visible_correction_hash_matches": (
            valid_result["correction"]["visible_message_sha256"]
            == authority[("QH10-E0-01", "C_align")]["content_sha256"]
        ),
        "policy_and_environment_auc_separate": (
            "policy_commitment_auc" in valid_result["metrics"]
            and "environment_debt_auc" in valid_result["metrics"]
            and "normalized_q_auc" not in valid_result["metrics"]
        ),
        ...
        "invalid_reply_side_effect_absent": not invalid_result["decisions"][0][
            "environment_debt"
        ],
