#!/usr/bin/env python3
"""Q-HIST v14 的冻结任务划分、A/R/E 单元与运行清单。"""
...
# “科学单元身份”只包含会改变实验含义的字段。执行顺序属于调度元数据，
# 不能进入科学身份，否则随机化调度会把同一单元伪装成不同实验对象。
SCIENTIFIC_UNIT_FIELDS = (
    "unit_id",
    "episode_id",
    "scenario",
    "family",
...
SCHEDULE_METADATA_FIELDS = ("execution_order",)


def scientific_unit_identity(row: dict[str, Any]) -> dict[str, Any]:
    """投影出可冻结、可跨调度比较的科学实验单元身份。"""

    missing = [field for field in SCIENTIFIC_UNIT_FIELDS if field not in row]
    if missing:
        raise ValueError(f"科学单元缺少身份字段：{missing}")
    return {field: deepcopy(row[field]) for field in SCIENTIFIC_UNIT_FIELDS}


def schedule_metadata(row: dict[str, Any]) -> dict[str, Any]:
    """提取不可混入科学身份、但必须独立审计的执行调度元数据。"""

    missing = [field for field in SCHEDULE_METADATA_FIELDS if field not in row]
    if missing:
        raise ValueError(f"实验单元缺少调度元数据：{missing}")
    execution_order = row["execution_order"]
    if isinstance(execution_order, bool) or not isinstance(execution_order, int):
        raise TypeError("execution_order 必须是正整数")
    if execution_order < 1:
        raise ValueError("execution_order 必须是正整数")
...
TECHNICAL_REPEAT_EPISODES = ("QH14-E0-01", "QH14-E0-03", "QH14-E0-05")
...
def build_scientific_trajectory_units() -> list[dict[str, Any]]:
    ...
    if len(units) != 168 or len({row["unit_id"] for row in units}) != 168:
        raise RuntimeError("v14 E0 科学清单必须含 168 条唯一轨迹")
    return units
...
def build_technical_repeat_units() -> list[dict[str, Any]]:
    ...
    if len(units) != 24 or len({row["unit_id"] for row in units}) != 24:
        raise RuntimeError("v14 E0 技术重复清单必须含 24 条唯一轨迹")
    return units
...
def build_trajectory_units() -> list[dict[str, Any]]:
    ...
    if len(units) != 192 or len({row["unit_id"] for row in units}) != 192:
        raise RuntimeError("v14 E0 必须含 192 条唯一轨迹")
    return units
...
def validate_frozen_design() -> dict[str, Any]:
    ...
    if counts != {"P00": 84, "P10": 24, "P01": 24, "P11": 60}:
        raise RuntimeError(f"v14 precision 计数漂移：{counts}")
    if requests != {"P00": 336, "P10": 96, "P01": 96, "P11": 240}:
        raise RuntimeError(f"v14 request 计数漂移：{requests}")
    if len(development_families) != 3 or len(confirmatory_families) != 8:
        raise RuntimeError("v14 必须冻结 3 个 development 与 8 个 confirmatory family")
    if len(CONFIRMATORY_SCENARIOS) != 16:
        raise RuntimeError("v14 confirmatory 必须含 16 个 episode")
    if development_families & confirmatory_families:
        raise RuntimeError("v14 development 与 confirmatory family 不得重叠")
    return {
