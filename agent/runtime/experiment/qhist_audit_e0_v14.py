#!/usr/bin/env python3
"""独立重算 Q-HIST v14 E0 的 F/V/G/B/E/T、clean、复现与资源门。"""
...
def json_domain(value: Any) -> Any:
    """按生产 JSON 边界规范化容器类型，不改变字段或数值。"""
...
def independently_build_schedule(
    units: list[dict[str, Any]], precision: str, seed: int
) -> list[dict[str, Any]]:
    """不导入生产 runner，独立复算确定性调度。"""

    selected = [dict(row) for row in units if row["precision"] == precision]
    ...
    if len(selected) != expected or len({row["unit_id"] for row in selected}) != expected:
        raise RuntimeError(f"{precision} 的资产单元数量或身份不唯一")
    ...
    return [
        dict(row, execution_order=index)
...
def verify_unit(
    ...
    expected_identity = spec.scientific_unit_identity(expected_unit)
    if artifact.get("status") != "succeeded":
        failures.append(f"{uid}:status")
        return {}, failures
    if spec.scientific_unit_identity(planned_unit) != expected_identity:
        failures.append(f"{uid}:planned-scientific-identity")
        return {}, failures
    if artifact.get("unit") != expected_identity:
        failures.append(f"{uid}:scientific-unit-identity")
        return {}, failures
    if artifact.get("schedule_metadata") != spec.schedule_metadata(planned_unit):
        failures.append(f"{uid}:schedule-metadata")
    ...
    official_record = json_domain(official)
    if official_record != json_domain(artifact.get("evaluation")):
        failures.append(f"{uid}:official-evaluator")
    return {
        "unit": expected_identity,
        "schedule_metadata": spec.schedule_metadata(planned_unit),
        "metrics": metrics,
        ...
        "official_similarity": float(official_record.get("similarity", 0.0)),
...
def main() -> int:
    ...
    parser.add_argument("--run-prefix", default="QHIST-E0-v14-traj")
    ...
    asset_units = load_jsonl(args.assets / "trajectory-units.jsonl")
    expected_units = {
        row["unit_id"]: spec.scientific_unit_identity(row)
        for row in spec.build_trajectory_units()
    }
    ...
    for precision in RUN_PRECISIONS:
        ...
        if driver_summary.get("seed") != spec.SCHEDULE_SEED:
            schedule_failures.append(f"{precision}:schedule-seed")
        recorded_schedule = load(driver / "schedule.json")
        expected_schedule = independently_build_schedule(
            asset_units, precision, spec.SCHEDULE_SEED
        )
        if recorded_schedule != expected_schedule:
            schedule_failures.append(f"{precision}:schedule-drift")
        for planned in recorded_schedule:
            uid = planned.get("unit_id")
            if not isinstance(uid, str) or uid in planned_units:
                schedule_failures.append(f"{precision}:duplicate-or-invalid-schedule-unit:{uid}")
                continue
            try:
                expected_identity = expected_units[uid]
                planned_identity = spec.scientific_unit_identity(planned)
                metadata = spec.schedule_metadata(planned)
            except (KeyError, TypeError, ValueError) as error:
                schedule_failures.append(f"{precision}:invalid-schedule-unit:{uid}:{type(error).__name__}")
                continue
            if planned_identity != expected_identity:
                schedule_failures.append(f"{precision}:scheduled-scientific-identity:{uid}")
            if metadata["execution_order"] > len(recorded_schedule):
                schedule_failures.append(f"{precision}:execution-order-range:{uid}")
            planned_units[uid] = planned
    ...
    failures = list(artifact_failures) + list(schedule_failures)
    failures.extend(f"missing:{uid}" for uid in missing)
    failures.extend(f"extra:{uid}" for uid in extra)
    verified_rows = []
    request_ids = []
    for uid in sorted(set(expected_units) & set(observed) & set(planned_units)):
        unit = expected_units[uid]
        artifact, _ = observed[uid]
        episode_id = unit["episode_id"]
        row, unit_failures = verify_unit(
            artifact, unit, planned_units[uid], episodes[episode_id], predicates[episode_id], patches[episode_id],
    ...
    implementation = {
        ...
        "deterministic_schedules_independently_recomputed": not schedule_failures and len(planned_units) == 192,
        "scientific_identity_separate_from_schedule_metadata": all(
            "execution_order" not in (artifact.get("unit") or {})
            and artifact.get("schedule_metadata") == spec.schedule_metadata(planned_units[uid])
            for uid, (artifact, _) in observed.items()
            if uid in planned_units
        ) and len(observed) == len(planned_units) == 192,
