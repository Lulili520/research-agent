#!/usr/bin/env python3
"""执行 Q-HIST v13：首决策前冻结 A/R/E checkpoint，随后恰好四个决策。"""
...
class DeterministicUUID4:
    ...
    def __call__(self) -> uuid.UUID:
        self.counter += 1
        if self.predecessor:
            payload = f"QHIST-v13|{self.logical_key}|predecessor|{self.counter}"
        else:
            payload = f"QHIST-v13|{self.logical_key}|target|{self.counter}"
...
def advance_decision(
    ...
    request_id = f"QH13-{row['precision']}-{row['unit_id']}-decision-{decision_index}"
...
def finalize(
    ...
    if len(decisions) != spec.DECISIONS_PER_TRAJECTORY:
        raise RuntimeError("v13 轨迹必须恰含四个目标模型决策")
...
def build_schedule(units: list[dict[str, Any]], precision: str, seed: int) -> list[dict[str, Any]]:
    ...
    if len(selected) != expected or len({row["unit_id"] for row in selected}) != expected:
        raise RuntimeError(f"{precision} 应含 {expected} 个 v13 单元，实际 {len(selected)}")
