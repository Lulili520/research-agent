#!/usr/bin/env python3
"""汇总 Q-HIST v14 的静态、reference、runner 和 ID 非补偿门。"""
...
def main() -> int:
    ...
    checks = {
        "asset_protocol_v14": asset_manifest.get("protocol_version") == 14,
        "rendered_protocol_v14": rendered_manifest.get("protocol_version") == 14,
        "all_gate_protocols_v14": all(
        ...
        "scripted_driver_gate_pass": (
            ...
            and driver.get("integration_scope", {}).get("json_persistence_round_trip_exercised") is True
            and all(
                ...
                and row.get("checks", {}).get("json_persistence_round_trip_audited") is True
    ...
    code_names = (
        ...
        "qhist_build_e0_assets_v14.py",
        "qhist_render_stimuli.py",
        "qhist_render_stimuli_v14.py",
        "qhist_toolsandbox_gate_v14.py",
        "qhist_trajectory_batch.py",
        "qhist_trajectory_batch_v9.py",
        "qhist_trajectory_batch_v14.py",
        "qhist_driver_integration_gate_v14.py",
        "qhist_deterministic_id_gate_v14.py",
        ...
        "qhist_model_server_v14.py",
        "qhist_combine_trajectory_batch.py",
        "qhist_combine_trajectory_batch_v14.py",
        "qhist_audit_e0_v14.py",
        "qhist_preflight_audit_v14.py",
        "qhist_verify_v14_lock.py",
        "run_qhist_v14_preflight.sh",
        "run_qhist_v14_qualification.sh",
        "run_qhist_v14_trajectory.sh",
        "run_qhist_v14_audit.sh",
        "test_qhist_v14_spec.py",
        "test_qhist_trajectory_batch_v14.py",
        "test_qhist_audit_e0_v14.py",
        "test_qhist_verify_v14_lock.py",
    )
    code = []
    missing = []
    ...
    checks["asset_manifest_sources_match_code"] = not missing and all(
        (
            asset_manifest.get("generator_sha256") == sha256(args.code_root / "qhist_build_e0_assets_v14.py"),
            asset_manifest.get("v14_spec_sha256") == sha256(args.code_root / "qhist_v14_spec.py"),
    ...
    checks["rendered_manifest_sources_match_code"] = not missing and all(
        (
            rendered_manifest.get("generator_sha256") == sha256(args.code_root / "qhist_render_stimuli_v14.py"),
            rendered_manifest.get("helper_sha256") == sha256(args.code_root / "qhist_render_stimuli.py"),
            rendered_manifest.get("v14_spec_sha256") == sha256(args.code_root / "qhist_v14_spec.py"),
        )
    )
    checks["driver_gate_sources_match_code"] = not missing and (
        driver.get("artifacts", {}).get("production_runner_sha256")
        == sha256(args.code_root / "qhist_trajectory_batch_v14.py")
        and driver.get("artifacts", {}).get("independent_auditor_sha256")
        == sha256(args.code_root / "qhist_audit_e0_v14.py")
        and driver.get("artifacts", {}).get("integration_gate_sha256")
        == sha256(args.code_root / "qhist_driver_integration_gate_v14.py")
    )
    checks["id_gate_source_matches_code"] = not missing and (
        ...
        == sha256(args.code_root / "qhist_deterministic_id_gate_v14.py")
    )
    checks["toolsandbox_gate_source_matches_code"] = not missing and (
        toolsandbox.get("gate_code_sha256")
        == sha256(args.code_root / "qhist_toolsandbox_gate_v14.py")
    )
