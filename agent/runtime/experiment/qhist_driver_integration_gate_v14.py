#!/usr/bin/env python3
"""用结果无关的脚本策略贯通 Q-HIST v14 production runner。"""
...
import qhist_audit_e0_v14 as independent_audit
...
def main() -> int:
    ...
    scheduled_units = runner.build_schedule(units, "P00", spec.SCHEDULE_SEED)
    unit_index = {
        (row["episode_id"], row["module"], row["condition"], int(row["gap"])): row
        for row in scheduled_units
    ...
    for episode_id, episode in episodes.items():
        for (module, condition), policy_name in condition_policy.items():
            ...
            checks = {
                ...
                "scientific_identity_matches_spec": (
                    result["unit"]
                    == spec.scientific_unit_identity(
                        expected_unit_index[(episode_id, module, condition, gap)]
                    )
                ),
                "execution_order_not_in_scientific_identity": (
                    "execution_order" not in result["unit"]
                ),
                "schedule_metadata_matches_scheduled_asset": (
                    result.get("schedule_metadata") == spec.schedule_metadata(row)
                ),
            }
            # 必须审计真实持久化边界，而不是只审计仍含 Python tuple/int-key
            # 的内存对象；这正是 v13 未覆盖的反例。
            ...
            checks["json_persistence_round_trip_audited"] = (
                persisted_result["evaluation"] == result["evaluation"]
    ...
    report = {
        ...
        "integration_scope": {
            ...
            "scheduled_rows_exercised": True,
            "scientific_identity_and_schedule_audited_separately": True,
            "json_persistence_round_trip_exercised": True,
        },
        "artifacts": {
