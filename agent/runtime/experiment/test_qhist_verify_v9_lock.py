...
import qhist_verify_v9_lock as verifier


class QHistVerifyV9LockTests(unittest.TestCase):
    def test_detects_code_drift(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            (runs / "rendered").mkdir(parents=True)
            (runs / "tool").mkdir(parents=True)
            (runs / "driver").mkdir(parents=True)
            targets = [
                code / "x.py",
