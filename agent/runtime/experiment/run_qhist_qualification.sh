...
workspace=/root/autodl-tmp/qhist-e0-v5
output="$workspace/runs/qualification/$precision"
...
model-env/bin/python code/qhist_precision_qualification.py \
  --precision "$precision" \
  ...
  --seed 20260903 \
  --max-new-tokens 128 \
  --output "runs/qualification/$precision" >"$log" 2>&1
status=$?
printf '%s\n' "$status" >"$exit_file"
exit "$status"
