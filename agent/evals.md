...
- 研究会话在第一次接触课题材料前绑定唯一 `RESEARCH_PROJECT_ROOT`；没有活动项目时不扫描 `research/` 寻找可复用历史。
- 处理项目 A 时，仓库级搜索排除 `research/**`，项目级搜索只包含 A；不得读取、概括或透露项目 B 的 topic、文件名、输出、日志和证据。
- 用户未明确要求跨项目导入时，不使用其他项目的查询、论文卡、候选方向、失败记录或实验结果初始化当前研究。
- 控制、审计或采集入口收到与活动范围不一致的项目目录时，在读取对方元数据前拒绝；对 A 的写入不会改变 B 的任何文件。
